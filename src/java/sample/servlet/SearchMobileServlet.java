    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (min.trim().length() > 0 && max.trim().length() > 0) {
                ...
                dao.searchFromStaffByPrice(Float.parseFloat(min), Float.parseFloat(max));
                List<MobileDTO> result = dao.getListMobiles();
                request.setAttribute("MOBILERESULT", result);
                url = userPage;
            }

        } catch (SQLException ex) {
            Logger.getLogger(SearchMobileServlet.class.getName()).log(Level.SEVERE, null, ex);
